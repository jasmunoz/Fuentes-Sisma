...
int InsertarRH(void* registro, long longi, char * valorSale); //PRJ0258542 - Fase 3
int ActualizaClaves(void* registro); //PRJ0258542
int ActualizaHSM(void* registro); //PRJ0258542
int tratar_unodos(char * opcion, char * clave, char * claves); //PRJ0258542
/*añadido para PRICE*/
int InsertarGT(void* registro, long longi, char * valorSale); //PRJ0385382
...
typedef struct formato_tabla_trms
{
 ...
 char	 trms_oper[2];				/* Tipo de operativa B = Bizum, T = Tarjeta */ //PRJ0387060
}      formato_tabla_trms;
...
typedef struct formato_tabla_para
{
  ...
  char ps_clrece[9];                                                                    
  char ps_clvhsm[51];
	char ps_clzmka[51];  //PRJ0258542
	char ps_clzmki[51];  //PRJ0258542
	char ps_clzika[51];  //PRJ0258542
	char ps_clziki[51];  //PRJ0258542
	char ps_clzpka[51];  //PRJ0258542
	char ps_clzpki[51];  //PRJ0258542
	char ps_clzaka[51];  //PRJ0258542
	char ps_clzaki[51];  //PRJ0258542
	char ps_clzeka[51];  //PRJ0258542
	char ps_clzeki[51];  //PRJ0258542
  char ps_clclap[51];  //PRJ0258542
  char ps_clblop[51];  //PRJ0258542
} formato_tabla_para;
...
EXEC SQL BEGIN DECLARE SECTION; 
   ...
   char errorn[4]; //PRJ288711 - opcion de llamada al procedimiento
	...
	char entradaU[100]; //PRJ0258542
	char salidaU[301]; //PRJ0258542
	char i_comando1[50]; //PRJ0258542
	char i_comando2[50]; //PRJ0258542
	char s_clv1[150]; //PRJ0258542
	char s_clv2[150]; //PRJ0258542
	
	char cadenaEntrada1[17]; //PRJ0258542 - Fase 3 - Cambio 11052026 - aumenta a 17
	char cadenaEntrada2[451]; //PRJ0258542
	char Entidad[5]; //PRJ0258542
	char Valor[3]; //PRJ0258542
	char Accion[2]; //PRJ0258542
	char Cadenaerr[101]; //PRJ0258542
	char identifica[5]; //PRJ0258542 - Fase 3
	char valore[11]; //PRJ0258542 - Fase 3
	char registroe[4001]; //PRJ0258542 - Fase 3
	char estado[3]; //PRJ0258542 - Fase 3
	char IPentra[2]; //PRJ0258542 - Fase 3
	char valors[11]; //PRJ0258542 - Fase 3
   
	char clavegt[10]; //PRJ0385382
	char indicadorgt[2]; //PRJ0385382
	char infcidcifivgt[33]; //PRJ0385382
	char infcidmsgchavecgt[5]; //PRJ0385382
	char infcidcifmsggt[129]; //PRJ0385382 de 65 a 129
	char sigdatsigtypgt[3]; //PRJ0385382
	char sigdatsiggt[17]; //PRJ0385382
	char descrierrgt[33]; //PRJ0385382
...
/* añadido para PRICE*/
  ...
  char psclrece[9];                                                                    
  char psclvhsm[51];                                              
	char psclzmka[51];  //PRJ0258542
	char psclzmki[51];  //PRJ0258542
	char psclzika[51];  //PRJ0258542
	char psclziki[51];  //PRJ0258542
	char psclzpka[51];  //PRJ0258542
	char psclzpki[51];  //PRJ0258542
	char psclzaka[51];  //PRJ0258542
	char psclzaki[51];  //PRJ0258542
	char psclzeka[51];  //PRJ0258542
	char psclzeki[51];  //PRJ0258542
	char psclclap[51];  //PRJ0258542
	char psclblop[51];  //PRJ0258542
  }estrucPARA;                                                    
...
{
   	fprintf(stdout,"Dentro del filefun - tratar_fich\n");
    fflush(stdout);
    //printf("tratar_fich\n");
...
// * añadido para PRICE *//
	      ...
	      {
	      	printf(". . . . Entrando por NPTPARA de ESCRIBIR . . . .\n");
	      	
	      	ErrInsertar=ActualizaClaves(registro);
			    if(ErrInsertar)
			    	return(salir_rb(abroConexion, ErrInsertar, tabla, opcion));
				  else
				  	return(salir_cm(abroConexion, ErrInsertar, tabla, opcion));	  
	     	}
	      if (!strncmp(tabla, "NPTDHSM", 7))
	      {
	      	printf(". . . . Entrando por NPTDHSM de ESCRIBIR . . . .\n");
	      	//printf(" Registro antes de ActualizaHSM >%s<\n",registro);
	      	ErrInsertar=ActualizaHSM(registro);
			    if(ErrInsertar)
			    	return(salir_rb(abroConexion, ErrInsertar, tabla, opcion));
				  else
				  	return(salir_cm(abroConexion, ErrInsertar, tabla, opcion));	  
        ...
        if (!strncmp(CodError, "000", 3)) //retorno OK
        {
           ...
           for (;;)
           {
              ...
              if (!strncmp(tabla, "NPTTRMS", 7))
              {
                ...
                strncpy(tabla_trms[ll].trms_oper, estrucTRMS.trmsoper , 1); //PRJ0387060
                ll++;  
              }
			  ...
			  if (!strncmp(tabla, "NPTPARA", 7))
              {
                ...
                if (ll >2)
                {
                   ...
                   //return (101);
								...
								//PRJ0258542 - INI
								strncpy(tabla_para[ll].ps_clzmka, estrucPARA.psclzmka,51);
								strncpy(tabla_para[ll].ps_clzmki, estrucPARA.psclzmki,51);
								strncpy(tabla_para[ll].ps_clzika, estrucPARA.psclzika,51);
								strncpy(tabla_para[ll].ps_clziki, estrucPARA.psclziki,51);
								strncpy(tabla_para[ll].ps_clzpka, estrucPARA.psclzpka,51);
								strncpy(tabla_para[ll].ps_clzpki, estrucPARA.psclzpki,51);
								strncpy(tabla_para[ll].ps_clzaka, estrucPARA.psclzaka,51);
								strncpy(tabla_para[ll].ps_clzaki, estrucPARA.psclzaki,51);
								strncpy(tabla_para[ll].ps_clzeka, estrucPARA.psclzeka,51);
								strncpy(tabla_para[ll].ps_clzeki, estrucPARA.psclzeki,51);
								strncpy(tabla_para[ll].ps_clclap, estrucPARA.psclclap,51);
								strncpy(tabla_para[ll].ps_clblop, estrucPARA.psclblop,51);
								//PRJ0258542 - FIN
                ll++;  				
              }
...
int InsertarL(void* registro)
{
    ...
    NPSLOGO logo;
		...
		//printf(".... Cadena de entrada registro 1 >%s< >%d< \n", registro, strlen(registro));       // temporal

    //PRJ0247167: se modifica la concatenacion de string por una copia de un numero de posiciones fija debido a la
    //            diferencia de medida entre los campos reales de NPTLOGO y la estructura "formato_interno"    
    //QuitarCaracteres(registro, sizeof(NPSLOGO));
    QuitarCaracteres(registro, sizeof(logo));
		//printf(".... Cadena de entrada registro 2 >%s< >%d< \n", registro, strlen(registro));			 // temporal
    ...
    strncpy(formatoI, (char*)registro+112, sizeof(logo.loforitn)-1);
		//PRJ0258542 - quitar cadena erronea del final
		if (strncmp(formatoI + 2510, loclavpr, 35) != 0)
		{
			strncpy(formatoI + 2510, "                                   ", 35);
		}
		//PRJ0258542 - quitar cadena erronea del final
    formatoI[sizeof(formatoI)-1]='\0';
...
//PRJ0258542 - INI
int ActualizaClaves(void* registro)
{
	int cont=0;
	int errSQL=0;
	int tamano=0;
	char auxAccion[1];
	strncpy(cadenaEntrada1, registro, 16); //PRJ0258542 - Fase 3 - Cambio 11052026, aumenta a 16
	cadenaEntrada1[17] = '\0';
	strncpy(cadenaEntrada2, registro + 16, 450); //PRJ0258542 - Fase 3 - Cambio 11052026, aumenta a 16
	cadenaEntrada2[451] = '\0';
	strncpy(CodError,"000",3);
	CodError[3] = '\0';
	printf("...... Llamada al procedimiento CAMBIOCLAVES .-.-.- \n");
	printf("--- Parametrizacion de entrada 1>%.16s< ---\n", cadenaEntrada1);
	printf("--- Parametrizacion de entrada 2>%.450s< ---\n", cadenaEntrada2);
	EXEC SQL CALL CAMBIOCLAVES (:cadenaEntrada1, :cadenaEntrada2, :CodError);
	printf("--- Error de salida >%.3s< ---\n", CodError);
	if (sqlca.sqlcode != 0 && strncmp(CodError, "000", 3) != 0)
	{
		errSQL = sqlca.sqlcode;
		printf(".-.-.-.-.sqlcode tras llamada procedimiento >%d<\n", sqlca.sqlcode);
	}
	else
	{
		if (strncmp(CodError, "000", 3) == 0)
		{
			printf(".-.-.-.-.salida con error 000 del procedimiento, valor sqlcode >%d<\n", sqlca.sqlcode);
		}
		else
		{
			errSQL = -1;
			printf(".-.-.-.-.salida con error -1 del procedimiento, por error >%.3s<\n", CodError);
		}
	}
	return(errSQL);
}

int ActualizaHSM(void* registro)
{
	int cont=0;
	int errSQL=0;
	int tamano=0;
	char auxAccion[1];
	strncpy(Entidad, registro, 4);
	Entidad[5] = '\0';
	strncpy(Accion, registro + 4, 1);
	Accion[2] = '\0';
	strncpy(Valor, registro + 5, 2);
	Valor[3] = '\0';
	strncpy(CodError,"000",3);
	CodError[3] = '\0';
	strncpy(Cadenaerr, " ", 100);
	Cadenaerr[101] = '\0';
	printf("...... Llamada al procedimiento ACTUALIZANPTDHSM .-.-.- \n");
	printf("--- Parametrizacion de entrada >%.4s< >%.1s< >%.2s<  >%.3s<---\n", Entidad, Accion, Valor, CodError);
	EXEC SQL CALL ACTUALIZANPTDHSM (:Entidad, :Accion, :Valor, :CodError, :Cadenaerr);
	//EXEC SQL CALL ACTUALIZANPTDHSM ('0001', '1', '01', :CodError);
	printf("--- Error de salida >%.3s< ---\n", CodError);
	if (sqlca.sqlcode != 0 && strncmp(CodError, "000", 3) != 0)
	{
		errSQL = sqlca.sqlcode;
		printf(".-.-.-.-.sqlcode tras llamada procedimiento >%d< >%.100s<\n", sqlca.sqlcode, Cadenaerr);
	}
	else
	{
		if (strncmp(CodError, "000", 3) == 0)
		{
			printf(".-.-.-.-.salida con error 000 del procedimiento, valor sqlcode >%d< >%.100s<\n", sqlca.sqlcode, Cadenaerr);
		}
		else
		{
			errSQL = -1;
			printf(".-.-.-.-.salida con error -1 del procedimiento, por error >%.3s<, valor sqlcode >%d< >%.100s<\n", CodError, sqlca.sqlcode, Cadenaerr);
		}
	}
	return(errSQL);
}
...
int tratar_fich1(char* ficher, OPCION_FICH opcion, char * registro, long tamanoE, char * salida)
{
	printf("Dentro de tratar_fich1 20260224\n");
	fflush(stdout);
	printf("Valores de entrada: registro >%s<, tamanoE >%d<\n",registro,tamanoE);
	fflush(stdout);
	int contador=0;
	int ErrInsertar=0;
	int retorReg=0;  //0=devuelve registro, 1=no devuelve registro
	...
	while (contador == 0)
	{
		...
		if (sqlca.sqlcode == -1012)
		{
			printf("Dentro filefun.cpp, abro conexion\n");
			fflush(stdout);
		  abroConexion=1;
		  conexion_BBDD();
		}
		...
		{
			printf("Comprueba conexion sqlca.sqlcode >%d<\n", sqlca.sqlcode);
			fflush(stdout);
		}
		contador++;
	}
	printf(". . . . Entrando por NPTRHSM desde procedimiento . . . .\n");
	fflush(stdout);
	//printf("Valores de registro >%s< y tamano >%d<\n",registro,tamano);
	//fflush(stdout);
	strncpy(cadena, registro, tamano);
	cadena[tamano] = '\0';
	//printf(" Registro antes de ActualizaClaves >%.*s<\n", tamano, cadena);
	//fflush(stdout);
	ErrInsertar=InsertarRH(cadena, tamano, valorSal);
  ...
  if(ErrInsertar)
	...
	printf("Salida de tratar_fich1\n");
	fflush(stdout); 
	return(salir_cm(abroConexion,0, tabla, opcion));
} //FIN tratar_fich1

int tratar_fich2(char* ficher, OPCION_FICH opcion, char * registro, long tamanoE)
{
	printf("Dentro de tratar_fich2 20260519\n");
	printf("Valores de entrada: ficher >%s<\n", ficher);
	printf(" registro >%s<\n", registro);
	printf(" tamano   >%d<\n", tamanoE);
	int contador=0;
	int ErrInsertar=0;
	...
	ch04[3]='\0';
	ch05[4]='\0';
	strncpy(cadena, registro, tamanoE);
	cadena[tamanoE]='\0';
	memset(claveT, '\0', 200); 
	tamano = tamanoE;
	strncpy(tabla, (char *)ficher, 7);
	...
	ErrInsertar=ActualizaClaves(cadena);
  if(ErrInsertar)
  	return(salir_rb(abroConexion, ErrInsertar, tabla, opcion));
  else
  	return(salir_cm(abroConexion, ErrInsertar, tabla, opcion));	  
...
int InsertarRH(void* registro, long longi, char * valorSale)
{
    int cont=0;
    int errSQL=0;
		int tamano=0;
		...
		printf("...... Llamada al procedimiento ACTUALIZANPTRHSM .-.-.- \n");
		//printf(".... Cadena de entrada registro 1 >%.*s< con tamano >%d< \n", longi, registro, longi);
		strncpy(identifica, registro, 4);
		identifica[5] = '\0';
    strncpy(valore, registro + 4, 10);
    valore[11] = '\0';
		strncpy(Accion, registro + 14, 1);
		Accion[2] = '\0';
    strncpy(estado, registro + 15, 2);
    estado[3] = '\0';
    //printf("Valor registro 17 >%.1s<\n", registro + 17);
    strncpy(IPentra, registro + 17, 1);
    IPentra[2] = '\0';
    //printf("Valor IPentra >%.1s<\n", IPentra);
    strncpy(registroe, registro + 18, longi - 18);
    registroe[longi - 18] = '\0';
    strncpy(CodError,"000",3);
		CodError[3] = '\0';
		strncpy(valors, "0000000000", 10);
		valors[11] = '\0';
		
		printf(".... Cadena a procedimiento con identifica >%.4s< con valore >%.10s< con valors >%.10s< \n", identifica, valore, valors);
		printf("     con accion >%.1s< con coderror >%.3s< con estado >%.2s< por IP >%.1s< \n", Accion, CodError, estado, IPentra);
		printf("     con registroe >%.*s<\n", tamano - 18, registroe);
		EXEC SQL CALL ACTUALIZANPTRHSM (:identifica, :valore, :Accion, :registroe, :estado, :IPentra, :valors, :CodError);
		printf("--- Error de salida >%.3s< valor >%.10s< ---\n", CodError, valors);

		if (sqlca.sqlcode != 0 && strncmp(CodError, "000", 3) != 0)
		{
			errSQL = sqlca.sqlcode;
			//printf(".-.-.-.-.sqlcode tras llamada procedimiento >%d< >%.10s<\n", sqlca.sqlcode, valors);
		}
		else
		{
			if (strncmp(CodError, "000", 3) == 0)
			{
				//printf(".-.-.-.-.salida con error 000 del procedimiento, valor sqlcode >%d< >%.10s<\n", sqlca.sqlcode, valors);
				strncpy(valorSale, valors, 10);
			}
			else
			{
				errSQL = -1;
				printf(".-.-.-.-.salida con error -1 del procedimiento, por error >%.3s<, valor sqlcode >%d< >%.10s<\n", CodError, sqlca.sqlcode, valors);
			}
		}
...
//PRJ0385382 - INI
int tratar_fich3(char* ficher, OPCION_FICH opcion, char * registro, long tamanoE, char * salida)
{
	printf("Dentro de tratar_fich3 20260630\n");
	fflush(stdout);
	printf("Valores de entrada: registro >%s<, tamanoE >%d<\n",registro,tamanoE);
	fflush(stdout);
	int contador=0;
	...
	strncpy(cadena, registro, tamano);
	cadena[tamano] = '\0';
	ErrInsertar=InsertarGT(cadena, tamano, valorSal);
  ...
  if(ErrInsertar)
  	printf("Tendria que hacer rollback\n");
  else
		printf("Tendria que hacer commit\n");	  
  //if(ErrInsertar)
  //	return(salir_rb(abroConexion, ErrInsertar, tabla, opcion));
  //else
	//	return(salir_cm(abroConexion, ErrInsertar, tabla, opcion));	  
	
	printf("Salida de tratar_fich3\n");
	fflush(stdout); 
	//return(salir_cm(abroConexion,0, tabla, opcion));
	return(0);
} //FIN tratar_fich3

int InsertarGT(void* registro, long longi, char * valorSale)
{
    int cont=0;
    int errSQL=0;
		int tamano=0;
		...
		printf("...... Llamada al procedimiento MODIFGESTTSAS .-.-.- \n");
		//printf(".... Cadena de entrada registro 1 >%.*s< con tamano >%d< \n", longi, registro, longi);
		
		strncpy(clavegt, registro, 9);
		clavegt[10] = '\0';
    strncpy(indicadorgt, registro + 9, 1);
    indicadorgt[2] = '\0';
    if (strncmp(indicadorgt, "T", 1) == 0)
    {
			strncpy(infcidcifivgt, registro + 10, 32);
			infcidcifivgt[33] = '\0';
	    strncpy(infcidmsgchavecgt, registro + 42, 4);
	    infcidmsgchavecgt[5] = '\0';
	    strncpy(infcidcifmsggt, registro + 46, 128); //30062026 cambia de 64 a 128
	    infcidcifmsggt[129] = '\0';
	    strncpy(sigdatsigtypgt, registro + 174, 2); //30062026
	    sigdatsigtypgt[3] = '\0';
	    strncpy(sigdatsiggt, registro + 176, 16);
	    sigdatsiggt[17] = '\0';
	    strncpy(descrierrgt, "                                ", 32);
	    descrierrgt[33] = '\0';
    }
    else
    {
			strncpy(descrierrgt, registro + 10, 32);
			descrierrgt[33] = '\0';
			strncpy(infcidcifivgt, "                                ", 32);
			infcidcifivgt[33] = '\0';
	    strncpy(infcidmsgchavecgt, "    ", 4);
	    infcidmsgchavecgt[5] = '\0';
	    strncpy(infcidcifmsggt, "                                                                                                                                ", 129);
	    infcidcifmsggt[129] = '\0';
	    strncpy(sigdatsigtypgt, "  ", 2);
	    sigdatsigtypgt[3] = '\0';
	    strncpy(sigdatsiggt, "                ", 16);
	    sigdatsiggt[17] = '\0';
    }
    strncpy(CodError,"000",3);
		CodError[3] = '\0';
		...
		printf(".... Cadena a procedimiento con infcidcifivgt >%.32s< con infcidmsgchavecgt >%.4s< \n", infcidcifivgt, infcidmsgchavecgt);
		printf("     con infcidcifmsggt >%.128s< con sigdatsigtypgt >%.2s< \n", infcidcifmsggt, sigdatsigtypgt);
		printf("     con sigdatsiggt >%.16s< con descrierrgt >%.32s< \n", sigdatsiggt, descrierrgt);
		EXEC SQL CALL MODIFGESTTSAS (:clavegt, :indicadorgt, :infcidcifivgt, :infcidmsgchavecgt, :infcidcifmsggt, :sigdatsigtypgt, :sigdatsiggt, :descrierrgt, :CodError);
		printf("--- Error de salida >%.3s< ---\n", CodError);
...
int leer_fich(char* ficher, OPCION_FICH opcion, void * registro, void * clave)
{
	fprintf(stdout,"Dentro del filefun - leer_fich\n");
	fflush(stdout);
	int contador=0;
	int ErrInsertar=0;
	int retorReg=1;  //0=devuelve registro, 1=no devuelve registro
	int abroConexion=0;
	...
	if (!strncmp(tabla, "NPTIDEN", 7))
	{
		fprintf(stdout,"leer_fich llamadaProc2 claveT>%s< tabla %s, opcion %s\n", claveT, (char*)tabla, (char*)opcionT);
		fflush(stdout);
		strncpy(CodError, "101", 3);
    contador=0;
    EXEC SQL ALLOCATE :registro3;
    EXEC SQL CALL llamadaProc2 (:opcionT, :tabla, :claveT, :registro3, :CodError); 
    EXEC SQL WHENEVER NOT FOUND DO break; 
	...
	else
	{
	  ...
	  EXEC SQL CALL LlamadaProc (:opcionT, :tabla, :claveT, :registro2, :CodError); 
		...
		if (sqlca.sqlcode != 0) 
		{
			//if ((sqlca.sqlcode != -1405) || ((sqlca.sqlcode == -1405) && (retorReg==0)))
			if ((sqlca.sqlcode != -1405) || ((sqlca.sqlcode == -1405) && (retorReg!=0)))
		 	{
		  	printf("error en el acceso LlamadaProc %i\n", sqlca.sqlcode );
				return (retorReg);
	...
	fprintf(stdout,"Saliendo del filefun - leer_fich >%i<\n",retorReg);
	fflush(stdout);
	return(retorReg);
} //FIN leer_fich
