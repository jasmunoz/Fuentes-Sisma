...
//int tratar_unodos(char * opcion, void * clave, char * registro); //PRJ0258542
/*añadido para PRICE*/
...
/* añadido para PRICE*/
  ...
  char psclvhsm[51];                                              
	...
	char dobleC[100]; //PRJ0258542
	char regsal[300]; //PRJ0258542                                                                  
...
//* añadido para PRICE*//
   if (opcion == CARGAR )
   {
	    ...
	    fprintf(stdout,"Dentro if CARGAR \n");
      fflush(stdout);
      printf("tabla %s \n",tabla);
      printf("claveT %s \n",claveT);
			fflush(stdout);
...
//PRJ0258542 - INI
int tratar_unodos(char * opcion, void * clave, char * registro)
{
	int contador=0;
	int ErrInsertar=0;
	int retorReg=0;  //0=devuelve registro, 1=no devuelve registro
	...
	fprintf(stdout,"Dentro de tratar_unodos\n");
  fflush(stdout);
	...
	ll = 0;
  strncpy(dobleC, " ", 100); 
	strncpy(cadenasal, " ",300);

	fprintf(stdout,"Opcion de entrada >%.3s<\n",opcion);
	fflush(stdout);
	if (strncmp(opcion, "UNO", 3) == 0)
	{
  	printf("Entra por UNO\n");
  	fflush(stdout);
		strncpy (dobleC, clave, 100);
		dobleC[100] = '\0';
		retorReg=0;
  	fprintf(stdout,".-.-Antes de llamada procedimiento 1\n");
    fflush(stdout);
    strncpy(regsal, " ", 300);
    regsal[300];
		switch (ll)
		{
			case 0:
				EXEC SQL CALL UNONPTIDEN (:dobleC, :regsal, :CodError);
  			break;
  	}
  	fprintf(stdout,".-.-.-.-.sqlcode tras llamada procedimiento 1>%d<\n", sqlca.sqlcode);
    fflush(stdout);
    strncpy(cadenasal, regsal, 300);
    cadenasal[300] = '\0';
    fprintf(stdout,".-.-.Salida de procedimi 1 \n");
    fflush(stdout);
    strncpy(registro, cadenasal, 300);
    registro[300] = '\0';
	}
	if (strncmp(opcion, "DOS", 3) == 0)
	{
		fprintf(stdout,"Entra por DOS\n");
		strncpy (claveT, clave, (strlen(clave)));
		claveT[strlen(clave)] = '\0';
		retorReg=0;
  	fprintf(stdout,".-.-Antes de llamada procedimiento 2\n");
    fflush(stdout);
    strncpy(registro2, " ", 300);
		switch (ll)
		{
			case 0:
		  	EXEC SQL CALL DOSNPTIDEN (:claveT, :registro2, :CodError);
		  	break;
	  }
  	fprintf(stdout,".-.-.-.-.sqlcode tras llamada procedimiento 2>%d<\n", sqlca.sqlcode);
    fflush(stdout);
    strncpy(registro, " ", 300);
    strncpy(registro, registro2, 150);
    strncpy(registro + 150, registro2 + 150, 150);
    registro[300] = '\0';
    fprintf(stdout,".-.-.Salida de registro 2 >%.300s<\n", registro);
    fflush(stdout);
	}
	fprintf(stdout,"----- Salida tratar_unodos -----\n");
	fflush(stdout);
	return (0);
...
int ActualizaClaves(void* registro)
{
	...
	int tamano=0;
	char auxAccion[1];
	char cadenaEntrada1[15];
	char cadenaEntrada2[450];
	strncpy(cadenaEntrada1, registro, 15);
	cadenaEntrada1[15] = '\0';
	strncpy(cadenaEntrada2, registro + 15, 450);
	cadenaEntrada2[450] = '\0';
	printf("...... Llamada al procedimiento CAMBIOCLAVES .-.-.- \n");
	printf("--- Parametrizacion de entrada >%.15s< ---\n", cadenaEntrada1);
	printf("--- Parametrizacion de entrada >%.450s< ---\n", cadenaEntrada2);
