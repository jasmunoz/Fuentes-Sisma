...
int main(int argc, char *argv[])
{
  ...
  freopen(fichero, "a", stdout); //a = modo de escritura y lectura, abre el creado o crea un nuevo
	...
	if (argc != 2)
	{
		printf("ControlHSM, introducir tiempo para lanzar cada 2 min, 3 min, 5 min, etc hasta 10 min\n");
		exit(-1);
	} 
	else 
	{
		...
		fprintf(stdout," \n");
		escribetraza("* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *");
		strncpy(printable,"Relanzar cada ", 14);
		strncpy(printable+14, lanzar, 2);
		strncpy(printable+16, " minutos", 8);
	...
	while (1)
	{
	  ...
	  strncpy(ip_conex2, reg_hsm.dhnumip2, 15);
		...
		if (estado_sal == 1)
		{
			escribetraza("* * * Ha fallado la conexion, no hace espera, relanza");
			estado_sal = 0;
		}
		else
		{
			escribetraza("* * * Espera tiempo establecido por parametro de entrada"); //en TMP en segundos de OWN_SIAM_ETS.SIAM_DHSM");
			fflush(stdout);
			sleep(segundos);
			estado_sal = 0;
		}
